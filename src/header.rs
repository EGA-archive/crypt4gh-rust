...
fn decrypt_packet(packet: &Vec<u8>, keys: &Vec<Keys>, sender_pubkey: &Option<Vec<u8>>) -> Option<Vec<u8>> {
	...
	eprintln!("Header Packet Encryption Method: {}", packet_encryption_method);
...
fn decrypt_x25519_chacha20_poly1305(
	...
	eprintln!("    my secret key: {:x?}", &privkey[0..32]);
	...
	eprintln!("    peer pubkey: {:x?}", peer_pubkey);
	eprintln!("    nonce: {:x?}", nonce.0);
	eprintln!("    encrypted data ({}): {:x?}", packet_data.len(), packet_data);
...
fn parse_edit_list_packet(packet: Vec<u8>) -> Vec<u64> {
	...
	eprintln!("Edit list length: {}", nb_lengths);
	eprintln!("packet content length: {}", packet.len() - 4);
